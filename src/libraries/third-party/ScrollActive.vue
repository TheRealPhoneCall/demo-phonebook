  ...
  methods: {
    ...
    getItemInsideWindow () {
      ...
      [].forEach.call(this.items, (item) => {
        ...
        const target = document.getElementById(decodeURI(item.getAttribute('link').substr(1)))
    ...
    handleClick (event) {
      ...
      const hash = event.currentTarget.getAttribute('link')
      ...
      this.scrollTo(target)
        .then(() => {
          if (!this.alwaysTrack) {
            this.scrollContainer.addEventListener('scroll', this.onScroll)
            const findClickedItem = item => decodeURI(item.getAttribute('link').substr(1)) === target.id
    ...
    scrollToHashElement () {
      ...
      const hash = window.location.href.match(/#[a-z-]+/g)
      // console.log('hash: ', hash)
      if (!hash) return

      const hashElement = document.querySelector(decodeURI(hash[0]))
